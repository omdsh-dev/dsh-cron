# dsh-cron Contributor Notes

This repository is a standalone DeepSeek Harness plugin: cross-session scheduled tasks (cron).
...
- Keep host-provided runtime APIs as optional peer dependencies on the scoped `@deepseek-ai/*` line; never mix the unscoped `cordis` package in.
- The durable job store (`src/store.ts`) is the source of truth; timers and dispatch bookkeeping are disposable projections.
