...
在 `web` profile 中，侧边栏时钟按钮打开定时任务中心：新建绑定当前会话的周期或单次任务、按状态筛选、查看本地化的调度与运行详情，并可直接立即运行 / 暂停 / 删除，全程无需模型参与。界面经由 loopback `/cron` RPC 通道与主机通信；其他插件也可以通过插件提供的 `cron` 服务驱动同一存储。
